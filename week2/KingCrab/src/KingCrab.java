...
public class KingCrab extends Critter
{
    ...
    public boolean moveAway(Actor a) {
        ArrayList<Location> location = getGrid().getEmptyAdjacentLocations(a.getLocation());

        for (Location l : location) {
            if (isDistance2(getLocation(), l)) {
                a.moveTo(l);
                return true;
            }
        }
    ...
    public boolean isDistance2(Location l1, Location l2) {
        int x1 = l1.getRow();
        int y1 = l1.getCol();
        int x2 = l2.getRow();
        int y2 = l2.getCol();
